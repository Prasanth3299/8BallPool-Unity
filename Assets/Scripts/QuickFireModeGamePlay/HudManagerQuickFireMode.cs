...
{
    public class HudManagerQuickFireMode : MonoBehaviour
    {
        public GameManagerQuickFire gameManager;
        public RectTransform canvas;
        public CanvasInputPositionHandlerQuickFireMode canvasInputPositionHandler;
        ...
        public GameObject playerGuideAndRulesScreen;
        public GameObject startingPopup;
        public GameObject gameOverScreenPopup;
        public int ballPottedCount=0;
        ...
        private CueSpinSpotQuickFireMode cueSpinSpotPos;
        private AimingWheelQuickFireMode aimingWheelPos;
        ...
        void Start()
        {
            StartCoroutine( StartingPopupEnable());
            ...
            cueSpinSpotPos = cueSpinSpot.GetComponent<CueSpinSpotQuickFireMode>();
            aimingWheelPos = aimingWheel.GetComponent<AimingWheelQuickFireMode>();
        ...
        public void UpdatePlayerPottedBallsListDisplay()
        {
            int countList = boardManagerQuickFire.pottedBalls.Count;
            int value = countList % 5;
            for (int i = 0, j = countList - value; i < value ; i++ , j++)
            {
                string[] ballNumber = boardManagerQuickFire.pottedBalls[j].name.Split("_"[0]);
                ballsPotted[i].sprite= ballSprites[int.Parse(ballNumber[1]) - 1];
                ballsPotted[i].color = new Color(1, 1, 1, 1);
            }
        }
        public void ClearPlayerPottedBallsDisplay()
        {
            for (int i = 0; i < ballsPotted.Length;i++)
            {
                ballsPotted[i].sprite = null;
                ballsPotted[i].color = new Color(1, 1, 1, 0);
            }
        }
        ...
        public IEnumerator StartingPopupEnable()
        {
           
            startingPopup.SetActive(true);
            yield return new WaitForSeconds(3);
            startingPopup.SetActive(false);
        }
        public void OnRulesButtonClicked()
        {
            menuParent.SetActive(false);
            playerGuideAndRulesScreen.SetActive(true);
        }
        public void OnPlayButtonClicked()
        {
            playerGuideAndRulesScreen.SetActive(false);
        }
    }
}
